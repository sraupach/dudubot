...
module.exports = function util(client, container) {
    ...
    lib.epochtime = function (timeInms) {
        //var time = !time ? new Date() : time;
        moment.locale('de');
        var mtime = moment(timeInms).format('LLL');
        return mtime;
    };
    ...
    lib.vlog = function (msg, svr) {
        var server = !svr ? "Unknown-Server" : svr; //check for server
        console.log(chalk.blue('[voiceAnnouncer] ') + chalk.yellow(server) + ': ' + msg);
    };
